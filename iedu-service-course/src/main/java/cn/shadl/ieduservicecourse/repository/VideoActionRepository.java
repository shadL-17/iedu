...
public interface VideoActionRepository extends JpaRepository<VideoAction, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM (SELECT DISTINCT(lid) as lid, ROUND(SUM(lid)/lid) as num FROM video_action WHERE ACTION=:#{#action} AND lid IN(SELECT lid FROM lession WHERE chid IN (SELECT chid FROM chapter WHERE cid=:#{#cid})) group BY lid) AS lession_num ORDER BY num DESC LIMIT :#{#n}")
